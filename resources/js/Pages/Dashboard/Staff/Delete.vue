...
function submit() {
    ...
    router.post(route("admin.staff.destroy", props.staff.id),
        {
            _method: 'delete'
        }, 
        {
    ...
    <Modal :show="show" :closeable="true">
        ...
        <div v-if="!inProgress" class="p-6">
            <div class="flex gap-4 items-center mb-6">
                <div class="h-24">
                    <img :src="staff.image" class="h-full" >
                </div>
                <div class="grow">
                    <h3 class="uppercase font-bold text-blue-800">{{ staff.name }}</h3>
                    <h2 class="text-red-600">Are you sure you want to DELETE this staff member?</h2>
                </div>
            </div>
