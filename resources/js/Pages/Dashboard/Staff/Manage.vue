...
const deleteSelectedImages = ()=>{
    ...
    router.post(route('admin.staff.images.destroy'), {
        _method: 'delete',
...
const fetchImages = async ()=>{
    ...
    {
        imageable_id: props.staff.data.id,
...
<template>
<Head title="Manage Staff Member" />

<DashboardLayout>
    <div class="flex justify-between items-center mb-2">
        <h2 class="uppercase">{{ staff.data.title }} {{ staff.data.name }}</h2>
        <Link :href="route('admin.staffs.index')" :class="btnClasses">Back</Link>
    </div>

    <div class="container mx-auto sm:px-6 lg:px-8 pb-6">

        <h2 class="sr-only">{{ staff.data.name }}</h2>
        
        <div class="container mx-auto px-3 md:px-0 grid gap-4 grid-cols-1 md:grid-cols-[2fr_2fr] mb-6 ">
            <div class="">
                <div class="w-full overflow-hidden bg-slate-200 p-2 border border-slate-500 border-dashed rounded-lg">
                    <img :src="staff.data.image" class="object-cover w-full max-h-screen" >                    
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="p-5">
                    <div class="mb-3">
                        <h3 class="text-xl font-bold text-blue-800 uppercase">{{ staff.data.title }} {{staff.data.name}}</h3>
                        <hr>
                    </div>
                    <h4>About.</h4>
                    <p class="border border-slate-400 border-dashed rounded-md p-5 mb-3" v-html="staff.data.description"></p>
                    <div class="">
                        <Edit :roles="roles" :staff="staff.data" />
                    </div>
                </div>
        ...
        <div class="">
            <div class="flex justify-between">
                <AddImages 
                    :model="staff.data" 
                    :customroute="'admin.staff.images.store'"
                    ...
                    <div class="rounded relative" :class="{'border-4 border-blue-600 shadow order-opacity-75': selectedImages.includes(image.id)}">
                        ...
                        <div class="absolute bottom-0 w-full p-1">
                            <div class="flex justify-end gap-1">
                                <EditImage :image="image" :customroute="'admin.staff.image.update'"/>
                                <DeleteImage 
                                    :image="image" 
                                    :customroute="'admin.staff.image.destroy'"
